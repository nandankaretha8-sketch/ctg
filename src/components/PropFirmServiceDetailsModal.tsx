...
const PropFirmServiceDetailsModal: React.FC<PropFirmServiceDetailsModalProps> = ({
  ...
  const fetchServiceDetails = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/prop-firm-services/${serviceId}`, {
        headers: {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/prop-firm-services/${serviceId}/notes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
