...
const SignalPlans = () => {
  ...
  const fetchUserSubscriptions = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/signal-plans/user/subscriptions', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const isUserSubscribed = (planId: string) => {
    const isSubscribed = userSubscriptions.some(sub => {
      const planMatch = sub.signalPlan === planId || sub.signalPlan._id === planId;
      const statusMatch = sub.status === 'active';
      const dateMatch = new Date(sub.endDate) > new Date();
  ...
  const handlePlanAction = (plan: SignalPlan) => {
    ...
    if (isUserSubscribed(plan._id)) {
      // User is subscribed, redirect to chatbox
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center px-4">
        <div className="text-white text-lg sm:text-xl text-center">Loading signal plans...</div>
      </div>
    );
  }
        ...
        {plans.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {plans.map((plan) => (
              <Card 
                key={plan._id} 
                className={`bg-white/5 backdrop-blur-md border-white/10 hover:bg-white/8 transition-all duration-300 ${
                  plan.isPopular ? 'ring-2 ring-orange-500/50 scale-105' : ''
                }`}
              >
