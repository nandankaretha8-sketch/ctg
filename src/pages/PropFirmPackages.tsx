...
const PropFirmPackages = () => {
  ...
  useEffect(() => {
    console.log('PropFirmPackages - Component mounted/updated');
    console.log('PropFirmPackages - User state:', user);
    console.log('PropFirmPackages - Loading state:', loading);
    
    fetchPackages();
    if (user) {
      console.log('PropFirmPackages - User exists, fetching user services');
      fetchUserServices();
    } else {
      console.log('PropFirmPackages - No user, skipping user services fetch');
    }
  }, [user]);

  const fetchPackages = async () => {
    try {
      console.log('PropFirmPackages - Starting to fetch packages');
      setLoading(true);
      setError(null);
      const response = await fetch('http://localhost:5000/api/prop-firm-packages');
      
      console.log('PropFirmPackages - Packages API response status:', response.status);
      
      if (response.ok) {
        const data = await response.json();
        console.log('PropFirmPackages - Packages data received:', data);
        setPackages(data.data || []);
      } else {
        const errorText = await response.text();
        console.error('PropFirmPackages - Packages API error:', response.status, errorText);
        throw new Error(`Failed to fetch packages: ${response.status}`);
      }
    } catch (error) {
      console.error('PropFirmPackages - Error fetching packages:', error);
      setError((error as Error).message);
      toast.error('Failed to load prop firm packages: ' + (error as Error).message);
    } finally {
      console.log('PropFirmPackages - Setting loading to false');
      setLoading(false);
    }
  };
  ...
  const fetchUserServices = async () => {
    try {
      console.log('PropFirmPackages - Starting to fetch user services');
      const token = localStorage.getItem('token');
      if (!token) {
        console.log('PropFirmPackages - No token found, skipping user services fetch');
        return;
      }

      console.log('PropFirmPackages - Token found, making API call');
      const response = await fetch('http://localhost:5000/api/prop-firm-services/my-services', {
        headers: {
      ...
      console.log('PropFirmPackages - User services API response status:', response.status);

      if (response.ok) {
        const data = await response.json();
        console.log('PropFirmPackages - User services data received:', data);
        console.log('PropFirmPackages - User services raw data:', JSON.stringify(data.data, null, 2));
        setUserServices(data.data || []);
      } else {
        const errorText = await response.text();
        console.error('PropFirmPackages - User services API error:', response.status, errorText);
      }
    } catch (error) {
      console.error('PropFirmPackages - Error fetching user services:', error);
    }
  };
  ...
  const hasActiveService = (packageId: string) => {
    console.log('PropFirmPackages - hasActiveService called for packageId:', packageId);
    console.log('PropFirmPackages - userServices:', userServices);
    
    const result = userServices.some(service => {
      console.log('PropFirmPackages - Checking service:', service);
      console.log('PropFirmPackages - service.package:', service.package);
      console.log('PropFirmPackages - service.status:', service.status);
      
      return service.package && service.package._id === packageId && 
        ['pending', 'active'].includes(service.status);
    });
    
    console.log('PropFirmPackages - hasActiveService result:', result);
    return result;
  };
  ...
  console.log('PropFirmPackages - Render state:', { loading, packages: packages.length, userServices: userServices.length, user: !!user, error });

  if (loading) {
    console.log('PropFirmPackages - Rendering loading state');
    return (
  ...
  if (error) {
    console.log('PropFirmPackages - Rendering error state');
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
  ...
  console.log('PropFirmPackages - Rendering main content');
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="container mx-auto px-4 py-8">
        ...
        {packages.length === 0 ? (
          ...
          console.log('PropFirmPackages - Rendering packages:', packages.length),
          <>
            {/* Packages Grid */}
                ...
                <Card key={pkg._id} className="glass-card border-glass-border/30 relative">
                  ...
                  <CardContent className="p-6 pt-0">
                    ...
                    {(() => {
                      ...
                      console.log(`PropFirmPackages - Package ${pkg._id} has active service:`, hasActive);
                      return hasActive ? (
                        <Button
