...
const ChallengeDetails = () => {
  ...
  const fetchUserMT5Account = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/challenges/${challengeId}/account`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const fetchLeaderboard = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/leaderboard?challengeId=${challengeId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleMT5Setup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/challenges/${challengeId}/join`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        // Switch to account tab to show the details
        setActiveTab('account');
      } else {
        toast.error(data.message || 'Failed to complete MT5 setup');
      }
    } catch (error) {
      console.error('Error completing MT5 setup:', error);
  ...
  // Check if user has pending_setup status (has joined but no MT5 details)
  const hasPendingSetup = userParticipation && userParticipation.status === 'pending_setup' && !userParticipation.accountId;
          ...
          <CardHeader className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="text-center sm:text-right">
                ...
                <div className="text-gray-400 text-xs sm:text-sm">Account Size</div>
              </div>
            </div>
          </CardHeader>
        </Card>
