...
const Mentorships = () => {
  ...
  const fetchUserSubscriptions = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('User subscriptions fetched:', data.data?.length || 0, 'subscriptions');
        setUserSubscriptions(data.data || []);
      } else {
        console.error('Failed to fetch user subscriptions:', response.status, response.statusText);
      }
    } catch (error) {
  ...
  const isUserSubscribed = (planId: string) => {
    ...
    console.log(`Plan ${planId}: ${isSubscribed ? 'SUBSCRIBED' : 'NOT SUBSCRIBED'}`);
    return isSubscribed;
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="text-white text-xl">Loading mentorship plans...</div>
      </div>
    );
  }
