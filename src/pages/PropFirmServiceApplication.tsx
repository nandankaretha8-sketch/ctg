    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.data.isFree) {
          ...
          navigate(`/prop-firm-services/${packageId}/payment`, {
        ...
        // Check if it's a Confirmo configuration error
        if (errorData.message && errorData.message.toLowerCase().includes('confirmo')) {
      ...
      // Check if it's a network error or Confirmo-related error
      if (error.message && error.message.toLowerCase().includes('confirmo')) {
        toast.error('Payment gateway not configured. Please contact support.');
